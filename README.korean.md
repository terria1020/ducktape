...
옵션 `taping` 바인딩으로 일반 터미널에서는 F10으로 1번 bound 세션에 진입하고, tmux 안에서는 저장된 디렉토리별 에이전트 세션을 순환할 수 있습니다.
...
| `F10` | 일반 터미널 | `ducktape-taping` 목록의 1번 경로 세션 attach |
| `F10` | 에이전트 안 | bind된 다음 디렉토리 세션으로 순환 |
...
~/anywhere  $ F10  →  ducktape-taping 1번 세션으로 attach
```
...
ducktape-taping     # 번호 기반 bound 디렉토리 목록 관리
ducktape-jumping 1  # bound 목록의 1번 경로 세션 attach/switch
...
`ducktape-taping`은 번호가 붙은 디렉토리 목록을 원형 리스트처럼 관리합니다.
일반 터미널 F10은 1번 경로 세션에 붙고, tmux 안 F10은 저장된 순서대로 다음 디렉토리의 에이전트 세션으로 이동합니다.
...
- `clear`: 전체 목록 삭제
- `--show`: 저장된 순서와 `ducktape-jumping <번호>` 힌트 출력, 현재 디렉토리 표시

`ducktape-jumping <번호>`는 현재 디렉토리와 무관하게 해당 번호 경로의 세션에 붙습니다.
대상 디렉토리의 에이전트 세션이 꺼져 있으면 자동으로 다시 띄웁니다.
목록이 비어 있거나 번호가 범위를 벗어나면 현재 디렉토리 세션을 만들지 않고 실패합니다.

### 세션 바

각 ducktape 세션은 전체 디렉토리 경로 해시를 기준으로 tmux 하단 바 색상이 정해집니다.
오른쪽 상태 영역에는 현재 디렉토리와 다음 바인드 디렉토리도 함께 표시됩니다.

```text
curr: [my-project] next: [another-project]
```

다음 바인드 디렉토리가 없으면 `next: []`로 표시됩니다.
...
> **기존 설치 사용자 주의:** 이제 `install.sh`는 `~/.tmux.conf`의 ducktape 블록을 최신 버전으로 교체합니다. F10과 관련 tmux 설정을 갱신하려면 설치 스크립트를 다시 실행하세요.
...
~/.zsh/.ducktape-bindings      # 번호 기반 bound 디렉토리 목록 (선택)
